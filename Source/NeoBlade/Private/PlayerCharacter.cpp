...
void APlayerCharacter::InputJump(const struct FInputActionValue& inputValue)
{
    // 공중 공격 중에는 점프 금지
    ...
    // 지상 공격 중에는 점프 금지
    if (CombatComponent && CombatComponent->IsInAttackState())
        return;

    // ★★★ 회피 중 점프 금지 ★★★
    if (CombatComponent && CombatComponent->IsDodging())
    {
        UE_LOG(LogTemp, Warning, TEXT("Jump Blocked: Dodging"));
        return;
    }



    // 착지 직전 입력 처리(아직 Landed() 호출 안 된 경우)
    if (!GetCharacterMovement()->IsFalling() && JumpCount != 0)
    {
        JumpCount = 0;
        bIsDoubleJumping = false;
    }
...
void APlayerCharacter::EndLanding()
{
    GEngine->AddOnScreenDebugMessage(-1, 1.f, FColor::Blue,
        FString::Printf(TEXT("END LANDING → Walk ON | MaxSpeed=%f"),
            GetCharacterMovement()->MaxWalkSpeed));
