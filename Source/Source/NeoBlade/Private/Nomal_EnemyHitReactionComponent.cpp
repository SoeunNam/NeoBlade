...
void UNomal_EnemyHitReactionComponent::PlayHitReaction()
{
    ...
    if (AMeleeEnemy* PawnOwner = Cast<AMeleeEnemy>(Owner))
    {

        PawnOwner->PlayHitMontage();
    }
...
void UNomal_EnemyHitReactionComponent::LaunchIntoAir(AActor* Causer)
{
    ...
    {
        //에어본 공격이 더 잫맞게 하기위해 speed 0
        enemy->GetCharacterMovement()->MaxWalkSpeed = 0.f;
        enemy->PlayAirBorneHitMontage();
    }
    if (AADEnemy* enemy = Cast<AADEnemy>(GetOwner()))
    {
        //에어본 공격이 더 잫맞게 하기위해 speed 0
        enemy->GetCharacterMovement()->MaxWalkSpeed = 0.f;
        enemy->A_PlayAirBorneHitMontage();
    }
}
...
void UNomal_EnemyHitReactionComponent::SustainAirborneHit()
{
    ...
    if (AMeleeEnemy* enemy = Cast<AMeleeEnemy>(GetOwner()))
    {
        enemy->PlayAirAttackedHitMontage();
    }
}
...
void UNomal_EnemyHitReactionComponent::AirFinishHit()
{
    ...
    if (AMeleeEnemy* enemy = Cast<AMeleeEnemy>(GetOwner()))
    {
        enemy->PlayFinalAttackHitMontage();
    }
