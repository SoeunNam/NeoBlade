...
AActor* UTargetingComponent::FindBestTarget()
{
    ...
    for (auto& Hit : Overlaps)
    {
        ...
        //황재원 추가 HP가 0인적은 절대로 타겟팅 하지않는다 //20251208
        if (UHealthComponent* HP = Candidate->FindComponentByClass<UHealthComponent>())
        {
            if (HP->CurrentHP <= 0.f)
            {
                continue;   // ← 이게 핵심이다
...
void UTargetingComponent::MaintainTarget()
{
    ...
    if (CurrentTarget)
    {
        ...
        // ⭐⭐⭐ 추가 : 타겟의 HP 체크 → 죽었으면 타겟 해제 ⭐⭐⭐
        if (UHealthComponent* HP = CurrentTarget->FindComponentByClass<UHealthComponent>())
        {
            if (HP->CurrentHP <= 0.f)
            {
                HideMarker();
