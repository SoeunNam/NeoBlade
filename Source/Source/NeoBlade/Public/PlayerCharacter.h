...
class UGuardShieldComponent; // 가드 전용 쉴드 시스템
...
public:
	...
	// 패링 / 가드 Ctrl 키
	UPROPERTY(EditDefaultsOnly, Category = "input")
	class UInputAction* ia_Parry_Guard;
	...
	// 패링 / 가드 입력 이벤트 처리 함수
	void InputGuardPressed(const FInputActionValue& Value);
	void InputGuardReleased(const FInputActionValue& Value);
...
public:
	...
	float GuardHoldThreshold = 0.25f;    // 0.25초 미만: 패링 / 이상: 유지형 가드

	// ==========================
	// 가드 쉴드 .. 
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Component")
	UGuardShieldComponent* GuardShieldComp;
