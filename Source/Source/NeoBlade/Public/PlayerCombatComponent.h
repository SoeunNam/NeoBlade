...
public:
    ...
    //void TickAirborneHitCheck();    // 에어본 전용 BoxTrace
...
public:
    ...
    void StartGuard();
    void EndGuard();
    bool TryParry(AActor* Attacker); // DamageComponent 가 호출

    //bool IsGuarding() const { return CombatState == ECombatState::Guard; }
    // 가드 중인지
    ...
    float GuardStartTime = 0.f;
    float ParryWindowTime = 0.5f;   // 1초 패링 윈도우

    // PlayerCombatComponent.h

public:
    void RequestParry();   // ★ 이 라인 추가

public:


    // 패링 성공했을 때 슬로우 모션 지속시간
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Parry")
    float ParrySlowMotionDuration = 0.25f;

    // 패링 슬로우모션 종료 타이머 핸들
    FTimerHandle ParrySlowTimer;


    // 패링 슬로우 모션 종료
    void EndParrySlowMotion();
